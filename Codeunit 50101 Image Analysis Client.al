codeunit 50104 "Image Analysis Client"
{
    ...
    begin
        ...
        YearRecord := Date2DMY(Rec."Birth Date", 3);
        DetectedAge := Result.FaceAge(1);
        YearPicture := Date2DMY(Today, 3) - DetectedAge;
        AgeDifference := DetectedAge - ExpectedAge;
        if Abs(AgeDifference) >= 5 then begin
            if Rec.Gender = Rec.Gender::Male then begin
                if AgeDifference < 0 then
                    Error('This seems to be %1''s younger brother. %1 is born in %2, and the guy in the picture is born around %3.', Rec.FullName(), YearRecord, YearPicture)
                else
                    Error('Who is this? %1''s father? %1 is born in %2, and the guy in the picture is born around %3.', Rec.FullName(), YearRecord, YearPicture);
            end else begin
                if AgeDifference < 0 then
                    Error('This seems to be %1''s younger sister. %1 is born in %2, and the gal in the picture is born around %3.', Rec.FullName(), YearRecord, YearPicture)
                else
                    Error('Who is this? %1''s mother? %1 is born in %2, and the lady in the picture is born around %3.', Rec.FullName(), YearRecord, YearPicture);
            end;
        end;
